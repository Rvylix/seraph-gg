...
function ChapterCard({ chapter }: { chapter: Chapter }) {
  ...
  return (
    ...
    >
      {/* Artwork */}
      <div style={{ height: 110, position: "relative", overflow: "hidden", background: `linear-gradient(135deg, ${typeStyle.bg}, var(--hbr-surface))`, flexShrink: 0 }}>
        {chapter.image_url && (
        ...
        {!chapter.is_released && (
          <div style={{ position: "absolute", bottom: 8, right: 8 }}>
            <span style={{ fontSize: 8, padding: "2px 6px", borderRadius: 3, background: "rgba(0,0,0,0.6)", color: "var(--hbr-muted)" }}>Coming Soon</span>
          </div>
        )}
      ...
      <div style={{ padding: "10px 12px" }}>
        ...
        {/* Expanded — spoiler summary */}
        {expanded && chapter.is_released && chapter.spoiler_summary && (
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: "0.5px solid var(--hbr-border)" }} onClick={e => e.stopPropagation()}>
            <div style={{ position: "relative" }}>
              <div style={{
                fontSize: 11, color: "var(--hbr-muted)", lineHeight: 1.6,
              ...
              {!revealed && (
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <button onClick={() => setRevealed(true)} style={{ fontSize: 9, padding: "4px 12px", borderRadius: 3, cursor: "pointer", background: "rgba(204,34,34,0.15)", color: "var(--hbr-red)", border: "0.5px solid rgba(204,34,34,0.3)", fontWeight: 600 }}>
                    ⚠ Reveal Spoiler
                  </button>
                </div>
              )}
              {revealed && (
                <button onClick={() => setRevealed(false)} style={{ marginTop: 8, fontSize: 9, padding: "2px 8px", borderRadius: 3, cursor: "pointer", background: "transparent", color: "var(--hbr-muted)", border: "0.5px solid var(--hbr-border)", display: "block" }}>
                  Hide
                </button>
              )}
            </div>
        ...
        {expanded && !chapter.is_released && (
          <div style={{ marginTop: 10, paddingTop: 10, borderTop: "0.5px solid var(--hbr-border)" }}>
            <p style={{ fontSize: 11, color: "var(--hbr-muted)" }}>This chapter has not been released in Global yet.</p>
          </div>
        )}

        {expanded && chapter.is_released && !chapter.spoiler_summary && (
          <div style={{ marginTop: 10, paddingTop: 10, borderTop: "0.5px solid var(--hbr-border)" }}>
            <p style={{ fontSize: 11, color: "var(--hbr-muted)" }}>No summary added yet.</p>
          </div>
        )}
      </div>
    </div>
...
function TimelineRow({ label, color, chapters }: { label: string; color: string; chapters: Chapter[] }) {
  ...
  return (
    <div style={{ marginBottom: 40 }}>
      ...
      {/* Timeline line */}
      <div style={{ height: 1, background: `linear-gradient(to right, ${color}44, ${color}22 80%, transparent)`, marginBottom: 12 }} />
