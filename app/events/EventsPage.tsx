...
function EventCard({ event, position }: { event: Event; position: "top" | "bottom" }) {
  ...
  return (
    ...
    >
      {/* Artwork */}
      <div style={{ height: 110, position: "relative", overflow: "hidden", background: isLive ? "rgba(34,204,102,0.08)" : isUpcoming ? "rgba(200,160,80,0.08)" : "rgba(255,255,255,0.03)" }}>
        {event.image_url
        ...
        <div style={{ position: "absolute", top: 8, left: 8 }}>
          <span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 3, color: statusColor, fontWeight: 700, background: "rgba(0,0,0,0.6)" }}>
...
function TimelineRow({ label, color, events }: { label: string; color: string; events: Event[] }) {
  ...
  return (
    <div style={{ marginBottom: 48 }}>
      ...
      {/* Alternating timeline — use CSS grid rows */}
          ...
          {events.map((e, i) => {
            ...
            return [
              ...
              <div key={`top-${e.id}`} style={{ display: "flex", alignItems: "flex-end", paddingBottom: 8, paddingRight: 10 }}>
                {isTop ? <EventCard event={e} position="top" /> : null}
              </div>,

              /* Middle cell — dot + line */
              <div key={`mid-${e.id}`} style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                <div style={{ width: 10, height: 10, borderRadius: "50%", background: dotColor, border: "2px solid var(--hbr-bg)", zIndex: 2, position: "relative", flexShrink: 0 }} />
              </div>,

              /* Bottom cell */
              <div key={`bot-${e.id}`} style={{ display: "flex", alignItems: "flex-start", paddingTop: 8, paddingRight: 10 }}>
                {!isTop ? <EventCard event={e} position="bottom" /> : null}
              </div>,
            ];
          })}
        </div>
